...
describe Insertion do
  context 'non-decreasing' do
    ...
    it 'sorts a one element array' do
      expect(insert.sort([1])).to eq [1]
    end

    it 'sorts a two element array' do
      expect(insert.sort([2, 1])).to eq [1, 2]
    end

    it 'sorts a three element array' do
      expect(insert.sort([3, 2, 1])).to eq [1, 2, 3]
    end

    it 'sorts a five element array' do
      expect(insert.sort([5, 1, 4, 2, 8])).to eq [1, 2, 4, 5, 8]
    end

    it 'sorts a six element array' do
      expect(insert.sort([5, 2, 4, 6, 1, 3])).to eq [1, 2, 3, 4, 5, 6]
    end
  end
